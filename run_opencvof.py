...
for ID in ID_list:
    for cam in cam_list:
        ...
        out_dir = Path(f"D:/data/{thing}{ID}/cam0{cam}/opencvof")
        ...
        for n in range(N):
            ...
            flow = cv2.calcOpticalFlowFarneback(cv2.cvtColor(im1, cv2.COLOR_BGR2GRAY), cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY), None, 0.5, 3, 15, 3, 5, 1.2, 0)
            flow_mdic = {"flow": flow}
